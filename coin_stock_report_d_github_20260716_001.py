...
# ============================================================
# DEFAULT SEED DATA
#   Used ONLY the first time gATH_table.csv does not exist yet.
#   Once the CSV exists, it becomes the single source of truth
#   for COIN_LIST, STOCK_LIST, and ATH_DATA.
# ============================================================
_DEFAULT_COIN_LIST = [
...
_DEFAULT_ATH_DATA = {
    ...
    "ARM":   {"ath": 439.46,  "date": "06/18/2026"},
}

NUM_WEEKS    = 52
NUM_MONTHS   = 12
NUM_DAYS     = 30
EXCEL_FILE   = "market_data_dreport.xlsx"
SHEET_ROW    = "Row Report"
SHEET_COL    = "Column Report"
ATH_CSV_FILE = "dATH_table.csv"

# ============================================================
# ATH TABLE — CSV BACKED  (create / load / save)
# ============================================================

def create_ath_table_if_missing(filename):
    """
    Creates gATH_table.csv seeded with default data if it doesn't
    already exist on disk.
    """
    if os.path.exists(filename):
        return

    print(f"  [Info] '{filename}' not found — creating with default seed data ...")
    rows = []
    for coin in _DEFAULT_COIN_LIST:
        key  = coin.upper()
        info = _DEFAULT_ATH_DATA.get(key, {"ath": "", "date": ""})
        rows.append({
            "symbol": coin, "type": "crypto",
            "ath": info.get("ath", ""), "date": info.get("date", "")
        })
    for stock in _DEFAULT_STOCK_LIST:
        info = _DEFAULT_ATH_DATA.get(stock, {"ath": "", "date": ""})
        rows.append({
    ...
    try:
        with open(filename, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["symbol", "type", "ath", "date"])
            writer.writeheader()
            writer.writerows(rows)
        print(f"  [Info] Created '{filename}' with {len(rows)} entries.")
    except Exception as e:
        print(f"  [Warning] Failed to create '{filename}': {e}")


def load_ath_table(filename):
    """
    Reads gATH_table.csv (creating it first if missing) and returns:
      coin_list  - list of lowercase crypto slugs   (for CoinGecko API calls)
      stock_list - list of uppercase stock tickers
      ath_data   - dict keyed by UPPERCASE display name ->
                   {"ath": float or "N/A", "date": str or "N/A"}
    """
    create_ath_table_if_missing(filename)
    ...
    try:
        with open(filename, "r", newline="") as f:
            ...
            for row in reader:
                ...
                ath_raw  = (row.get("ath")  or "").strip()
                date_raw = (row.get("date") or "").strip()

                if not symbol or not typ:
                    continue

                try:
                    ath_val = float(ath_raw) if ath_raw != "" else "N/A"
                except ValueError:
                    ath_val = "N/A"
                ath_date = date_raw if date_raw != "" else "N/A"

                if typ == "crypto":
                    coin_list.append(symbol.lower())
                    key = symbol.upper()
                elif typ == "stock":
                ...
                ath_data[key] = {"ath": ath_val, "date": ath_date}

    except Exception as e:
        print(f"  [Warning] Failed to read '{filename}': {e}")

    return coin_list, stock_list, ath_data


def save_ath_table(filename, ath_data, coin_list, stock_list):
    """
    Rewrites gATH_table.csv fully from the current in-memory
    ATH_DATA / COIN_LIST / STOCK_LIST — preserving original ordering.
    Called at the end of a run so any newly-detected ATHs are persisted.
    """
    rows = []
    for coin in coin_list:
        key  = coin.upper()
        info = ath_data.get(key, {"ath": "", "date": ""})
        ath_out  = info.get("ath",  "")
        date_out = info.get("date", "")
        if ath_out  in ("N/A", None): ath_out  = ""
        if date_out in ("N/A", None): date_out = ""
        rows.append({"symbol": coin, "type": "crypto",
                     "ath": ath_out, "date": date_out})

    for stock in stock_list:
        info = ath_data.get(stock, {"ath": "", "date": ""})
        ath_out  = info.get("ath",  "")
        date_out = info.get("date", "")
        if ath_out  in ("N/A", None): ath_out  = ""
        if date_out in ("N/A", None): date_out = ""
        rows.append({"symbol": stock, "type": "stock",
                     "ath": ath_out, "date": date_out})
    ...
    try:
        with open(filename, "w", newline="") as f:
            ...
            writer.writeheader()
            writer.writerows(rows)
        print(f"  ✔ ATH table saved: '{filename}' ({len(rows)} entries)")
    except Exception as e:
        print(f"  [Warning] Failed to save '{filename}': {e}")


def update_ath_if_new_high(name, prices, current_price, ath_data, price_dates=None):
    """
    Checks the historical price series (and the live current_price) for a
    new all-time high versus what is stored in ath_data for `name`.
    If a new high is found, ath_data[name] is updated IN-PLACE with the
    new ATH value and the date it occurred (looked up from price_dates
    when available, otherwise defaults to today's date for the live-price
    case). Returns (ath_val, ath_date, is_new_high).
    """
    existing = ath_data.get(name, {})
    ath_val  = existing.get("ath",  "N/A")
    ath_date = existing.get("date", "N/A")

    if not prices:
        return ath_val, ath_date, False

    hist_max = max(prices)
    max_date = None
    if price_dates and len(price_dates) == len(prices):
        max_idx  = prices.index(hist_max)
        max_date = price_dates[max_idx]

    overall_max = hist_max

    # The live/current price can be newer than the last historical bar
    # (especially for stocks via fast_info) — check that too.
    if isinstance(current_price, (int, float)) and current_price > overall_max:
        overall_max = current_price
        max_date    = datetime.now()

    is_new_high = False
    if isinstance(overall_max, (int, float)):
        if ath_val == "N/A" or overall_max > ath_val:
            new_date_str = (max_date.strftime("%m/%d/%Y")
                            if isinstance(max_date, datetime)
                            else datetime.now().strftime("%m/%d/%Y"))
            ath_data[name] = {"ath": overall_max, "date": new_date_str}
            ath_val     = overall_max
            ath_date    = new_date_str
            is_new_high = True

    return ath_val, ath_date, is_new_high

# ============================================================
# LOAD THE LIVE COIN LIST / STOCK LIST / ATH DATA FROM CSV
#   These behave exactly like the old hardcoded module-level
#   constants — everything below still references them as
#   COIN_LIST / STOCK_LIST / ATH_DATA.
# ============================================================
COIN_LIST, STOCK_LIST, ATH_DATA = load_ath_table(ATH_CSV_FILE)
...
def values_are_equal(a, b):
    """
    Flexible numeric equality used for tracking-value mirror matching.
    Handles both plain floats and formatted strings like '0.00012345'.
    """
    def to_float(x):
    ...
    tolerance = max(abs(fa), abs(fb)) * 1e-5
...
def get_tracking_dates():
    """
    Generates date headers from 06/22/2026 up to today,
    reversed so newest date comes first (closest to day-bucket columns/rows).
    """
...
# ============================================================
# TRACKING VALUE → FORMAT LOOKUP BUILDER
# ============================================================

def build_tracking_format_lookup(r, tracking_dates):
    """
    Returns a list of dicts for every tracking date cell that has a
    non-empty value.  ALL entries with values are included — not just
    those that carry explicit formatting — so that mirror-matching works
    even when the tracking cell has default (no-color) formatting.

    Priority: newest date first (tracking_dates is already newest-first).
    First match wins during apply step.
    """
    lookup = []
    for d_str in tracking_dates:
        ...
        if not t_info:
            continue
        raw_val = t_info.get("value")
        if raw_val in ("", None):
            continue
        lookup.append({
            "value":      raw_val,
            "font_color": t_info.get("font_color"),
            "fill_color": t_info.get("fill_color"),
            "is_bold":    t_info.get("is_bold", False),
        })
    return lookup


def apply_tracking_mirror_to_cell(cell, cell_value, lookup):
    """
    If cell_value matches any entry in lookup, apply that entry's
    font/fill formatting to the cell.  First match wins.

    This OVERWRITES any previously applied font/fill on the cell so
    that tracking-date formatting always takes final precedence.
    """
    if cell_value in ("", None):
        return
    for entry in lookup:
        if values_are_equal(cell_value, entry["value"]):
            f_color  = entry.get("font_color")
            is_bold  = entry.get("is_bold", False)
            bg_color = entry.get("fill_color")

            # Always apply font (overwrite whatever was set before)
            if f_color and f_color not in ("00000000", "FF000000"):
                cell.font = Font(color=f_color, bold=is_bold)
            elif is_bold:
                cell.font = Font(bold=True)
            else:
                cell.font = Font(bold=cell.font.bold if cell.font else False)

            # Always apply fill when present
            if bg_color and bg_color not in ("", "00000000"):
...
def process_raw_data(name, prices, current_price,
                     ...
                     is_stock=False, price_dates=None):
    try:
        ...
        # ── Check for a new ATH and update the CSV-backed ATH_DATA ──
        ath_val, ath_date, is_new_high = update_ath_if_new_high(
            name, prices, current_price, ATH_DATA, price_dates)

        if is_new_high:
            print(f"    🚀 New ATH for {name}: {format_price(ath_val)} on {ath_date}")
...
# ============================================================
# SHARED: apply all per-asset cell formatting  (ROW layout)
# ============================================================
...
def apply_asset_formatting(ws, current_row, r, headers,
    ...
    tracking_lookup = build_tracking_format_lookup(r, tracking_dates)
    if tracking_lookup:
        base_len    = 30
        ...
        wk_start = base_len + 1
        wk_end   = base_len + weekly_len
        mo_start = wk_end + 1
        mo_end   = wk_end + monthly_len
        d1_start = idx_day_start_1
        d1_end   = idx_day_start_1 + NUM_DAYS - 1
        d2_start = idx_day_start_2
        d2_end   = idx_day_start_2 + NUM_DAYS - 1
        ...
        for (c_start, c_end) in scan_ranges:
            for col_idx in range(c_start, c_end + 1):
                cell       = ws.cell(row=current_row, column=col_idx)
                cell_value = cell.value
                apply_tracking_mirror_to_cell(cell, cell_value, tracking_lookup)
...
def write_sheet_row(ws, results):
    ...
    has_existing_data = (ws.max_row > 1 or
                         (ws.max_row == 1 and ws.cell(1, 1).value is not None))
    ...
    for i, r in enumerate(results):
        ...
        apply_asset_formatting(
            ...
            idx_min_mo12, idx_max_mo12,
            idx_day_start_1, idx_day_start_2,
            idx_tracking_start, center_cols,
...
def write_sheet_col(ws, results):
    ...
    wk_start_orig  = base_len + 1
    wk_end_orig    = base_len + weekly_len
    mo_start_orig  = wk_end_orig + 1
    mo_end_orig    = wk_end_orig + monthly_len
    d1_start_orig  = idx_day_start_1
    d1_end_orig    = idx_day_start_1 + NUM_DAYS - 1
    d2_start_orig  = idx_day_start_2
    d2_end_orig    = idx_day_start_2 + NUM_DAYS - 1

    scan_orig_ranges = [
        (wk_start_orig, wk_end_orig),
        (mo_start_orig, mo_end_orig),
        (d1_start_orig, d1_end_orig),
        (d2_start_orig, d2_end_orig),
    ]
    ...
    for asset_idx, r in enumerate(results):
        ...
        tracking_lookup = build_tracking_format_lookup(r, tracking_dates)
        if tracking_lookup:
            for (orig_start, orig_end) in scan_orig_ranges:
                for orig_col in range(orig_start, orig_end + 1):
                    cell       = ws.cell(row=get_t_row(orig_col), column=t_col)
                    cell_value = cell.value
                    apply_tracking_mirror_to_cell(cell, cell_value, tracking_lookup)
    ...
    for coin in COIN_LIST:
        ...
        for attempt in range(3):
            try:
                ...
                price_dates = [datetime.fromtimestamp(p[0] / 1000.0)
                              for p in data['prices']]

                res = process_raw_data(
                    coin_upper, prices, prices[-1],
                    previous_counts, previous_tracking_data,
                    is_stock=False, price_dates=price_dates)
                if res:
                    results.append(res)
                    print("OK")
                time.sleep(5)
...
def fetch_stock_data(previous_counts, previous_tracking_data):
    ...
    for stock in STOCK_LIST:
        ...
        try:
            ...
            price_dates   = [d.to_pydatetime() if hasattr(d, "to_pydatetime") else d
                             for d in hist.index]
            ...
            res = process_raw_data(
                ...
                previous_counts, previous_tracking_data,
                is_stock=True, price_dates=price_dates)
            if res:
                results.append(res)
...
def fetch_all_data():
    print("\n" + "=" * 65)
    print(f"  ATH table loaded from '{ATH_CSV_FILE}'")
    print(f"  Coins tracked : {len(COIN_LIST)}")
    print(f"  Stocks tracked: {len(STOCK_LIST)}")
    print("=" * 65)
    ...
    print("\n" + "=" * 65)
    print("  Persisting ATH table ...")
    print("=" * 65)
    save_ath_table(ATH_CSV_FILE, ATH_DATA, COIN_LIST, STOCK_LIST)

    total_metrics = len(build_excel_headers())
    print("\n✔ All done!")
    print(f"  Output file    : {EXCEL_FILE}")
    print(f"  ATH table file : {ATH_CSV_FILE}")
