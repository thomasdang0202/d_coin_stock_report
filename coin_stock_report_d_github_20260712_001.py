...
# ============================================================
# TRACKING VALUE → FORMAT LOOKUP BUILDER  (UPDATED)
# ============================================================
    ...
    for entry in lookup:
        if values_are_equal(cell_value, entry["value"]):
            ...
            if f_color and f_color not in ("00000000", "FF000000"):
                ...
                # No explicit tracking color — still mark the match
                # with bold so the mirroring is visible
...
def apply_asset_formatting(ws, current_row, r, headers,
    ...
    # ── 1. Default thin border on every data cell ────────────
    for col_idx in range(1, len(headers) + 1):
        ws.cell(row=current_row, column=col_idx).border = thin_border

    # ── 2. Week-grouping thick right borders (2nd daily group)
    ...
    # ── 6. Daily group 1 — month touch colours ───────────────
    ...
    # ── 7. Daily group 2 — week touch colours ────────────────
    ...
    # ── 9. Mirror tracking formatting onto matching
    #        week / month / day cells — runs LAST to take
    #        final precedence over steps 4-7 above ──────────
...
def write_sheet_col(ws, results):
    ...
    # Column ranges for mirror (1-based original col idx)
    ...
    for asset_idx, r in enumerate(results):
        ...
        # Tracking date cells — write their own format first
        if idx_tracking_start:
            for i, d_str in enumerate(tracking_dates):
        ...
        # ── Mirror tracking formatting onto matching week/month/day
        #    rows — runs LAST to take final precedence ───────────
        tracking_lookup = build_tracking_format_lookup(r, tracking_dates)
        if tracking_lookup:
